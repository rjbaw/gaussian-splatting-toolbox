...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, model_type : str):
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        if self.model_type == "of" or self.model_type == "gs2d":
            opacities = self.inverse_opacity_activation(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))
        if self.model_type == "gs3d":
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, grads_abs=None, grad_abs_threshold=None, N=2):
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        if self.model_type == "of":
            ...
            self.densify_and_clone(grads, max_grad, extent, grads_abs, Q)
            clone = self._xyz.shape[0]

            self.densify_and_split(grads, max_grad, extent, grads_abs, Q)
            split = self._xyz.shape[0]

        if self.model_type == "gs3d" or self.model_type == "gs2d":
