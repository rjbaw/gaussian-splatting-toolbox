...
def render_set(model_path,
    ...
    if model_type == "of":
        render_path = os.path.join(model_path,
                                name,
                                "ours_{}".format(iteration), f"test_preds_{scale_factor}")
        gts_path = os.path.join(model_path,
                                name,
                                "ours_{}".format(iteration), f"gt_{scale_factor}")
    if model_type == "gs3d":
...
def render_sets(dataset : ModelParams,
                ...
                model_type : str):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree, model_type)
