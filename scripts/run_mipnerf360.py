...
output_dir = "results/exp_360/release"
...
def train_scene(gpu, scene, factor):
    cmd = f"CUDA_LAUNCH_BLOCKING=1 OMP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES={gpu} python train.py -s data/360_v2/{scene} -m {output_dir}/{scene} --eval -i images_{factor} --port {6109+int(gpu)}"
    print(cmd)
    if not dry_run:
       os.system(cmd)
